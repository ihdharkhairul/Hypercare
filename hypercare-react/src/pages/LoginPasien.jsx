...
export default function LoginPasien() {
  ...
  const { loginPasien, loading } = useAuth();
  const navigate  = useNavigate();
  const location  = useLocation();

  // PERBAIKAN: Abaikan 'from' jika nilainya '/' atau halaman auth lain
  // Kalau user datang dari beranda atau langsung buka /login-pasien,
  // selalu arahkan ke dashboard — bukan balik ke beranda
  const rawFrom = location.state?.from?.pathname;
  const authPages = ['/', '/login-pasien', '/login-perawat', '/register-pasien', '/register-perawat'];
  const from = rawFrom && !authPages.includes(rawFrom) ? rawFrom : '/dashboard-pasien';
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await loginPasien(email, password, remember);
    ...
    <div className="font-body bg-secondary min-h-screen flex items-center justify-center p-4">
      <div className="card-soft w-full max-w-md p-8">
        <div className="text-center mb-7">
          ...
          <h1 className="font-display font-bold text-xl">Masuk sebagai Pasien</h1>
          <p className="text-sm text-[#64748B] mt-1">Pantau kesehatan Anda setiap hari</p>
        </div>
        ...
        <p className="text-center text-sm text-[#64748B] mt-5">
          Belum punya akun? <Link to="/register-pasien" className="text-primary font-semibold text-hover">Daftar</Link>
        </p>
        <p className="text-center text-xs text-[#94A3B8] mt-3">
          Login sebagai perawat? <Link to="/login-perawat" className="text-primary font-semibold text-hover">Klik di sini</Link>
        </p>
      </div>
    </div>
  );
